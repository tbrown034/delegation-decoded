...
export function MemberCoverageBar({ coverage }: MemberCoverageBarProps) {
  return (
    <div className="flex flex-wrap gap-x-3 gap-y-0.5 font-mono text-[10px]">
      {memberCoverageSources.map(({ key, label }) => {
        ...
        return (
          <span key={key} className={color}>
            <span
              aria-hidden="true"
              className={`mr-0.5 inline-block size-1 rounded-full ${
                status === "good"
                  ? "bg-emerald-600"
                  : status === "partial"
                    ? "bg-amber-500"
  ...
  return (
    <div className="border-t border-neutral-100 pt-4">
      ...
      <div className="space-y-0.5">
        {notes.map((note) => (
          <p key={note} className="text-[11px] text-neutral-400">
            {note}
          </p>
        ))}
      </div>
