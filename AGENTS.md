...
scripts/ingest/
  ...
  elections.ts            State-authority race adapter runner. Indiana backfills a completed primary; Delaware and Florida track current qualifying status, including inactive history. Private Blob snapshots precede append-only status/result writes. FEC remains the labeled fallback where no adapter is covered.
