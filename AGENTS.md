...
scripts/ingest/
  ...
  elections.ts            State-authority race adapter runner. Indiana backfills a completed primary; Delaware, Florida and Rhode Island track current qualifying status, including inactive history. Nebraska reconciles its current federal list against certified primary results and a separately certified petition candidate. Rhode Island and Nebraska reach verified-ballot coverage. Private Blob snapshots precede append-only status/result writes. FEC remains the labeled fallback where no adapter is covered.
