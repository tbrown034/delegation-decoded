...
export async function getMemberBiographyHealth(): Promise<MemberBiographyHealth> {
  try {
    const result = await db.execute(sql`
      ...
      SELECT
        (SELECT COUNT(DISTINCT bioguide_id)::int FROM eligible) AS members_with_eligible_quotes,
        (SELECT COUNT(*)::int FROM eligible) AS eligible_quote_rows,
    ...
    return {
      ...
      membersWithEligibleQuotes: Number(row?.members_with_eligible_quotes ?? 0),
      eligibleQuoteRows: Number(row?.eligible_quote_rows ?? 0),
