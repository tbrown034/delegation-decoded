...
- Sunday morning runs: daily 06:23 UTC success; the Nebraska adapter failed closed because the state's list gained two convention-nominating parties and a second certified petition candidate (Mark Cohen); adapter now admits convention parties as `state_general_list` and keeps a registry of certification pages. Weekly 07:17 UTC success in 65 minutes, the first without press releases. The new drop counter reported 89 of 108 campaign items refused, five sites at 100%: the model wraps quotes in curly quotation marks and uses straight apostrophes, and the strict substring check rejected both. The guard now strips wrapping marks, folds typography for the comparison, and publishes the page's exact span; a first backfill pass published 61 records with 3 genuine splice rejections.
