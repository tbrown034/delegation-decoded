...
## 2026-09-06 — Overnight Texas-page fixes, Sunday runs, verbatim guard repair

**Session Summary:**
- Live Texas pages (Grok cold-read): FEC-only race pages omitted sitting members with no Form 2 (TX-09 listed 13 filers and no Al Green). The FEC fallback now adds the officeholder as a labeled `sitting_member_no_2026_fec_filing` reference row; the race-page ask blurb says "FEC filings" on those contests; committee chips use the roster `last_name` (De La Cruz read "Cruz", Weber read "Sr."); FEC names re-normalized so suffixes trail the surname and typed-in titles are dropped (new `scripts/lib/fec-names.ts`, 2,498 rows re-ingested, answer cache flushed); duplicate FEC IDs listed once.
- Biography classifier ignores a sentence's leading title ("Congressman Al Green co-founded ... law firm" is career, not public service) and recognizes "term in the United States House"; 8,662 facts reclassified deterministically.
- Ask audit rows are written through Next's `after()`: a live 422 signature probe had been lost because the function froze before the fire-and-forget insert landed. Re-probed after deploy; the row is there. Anthropic `max_tokens` raises provider-unavailable; provider budget is checked before the global counter. Prompt v10 asks for each ref once instead of repeating a set after every sentence.
- /for-journalists gained an AI disclosure; /about anchors its AI section. Delegation briefs regenerated (Texas read "Generated Aug 23"). Interview-prep files that named the hiring panel were moved out of this public repo; they remain in history.
- Sunday daily run (06:23 UTC) succeeded. Nebraska failed closed because the state's filing list gained two convention-nominating parties (Nebraska Working People, America First) and a second certified petition candidate (Mark Cohen, NE-03, Aug 12). Parties with no primary records are admitted from the state list as `state_general_list`; petition certifications are now a registry of Secretary of State pages, each fetched, validated and snapshotted.
- Sunday weekly run (07:17 UTC) succeeded in 65 minutes, the first without the press-release step; issues #2-#4 stay closed.
- The drop counter added Saturday reported the verbatim guard refusing 89 of 108 campaign items, five sites at 100%. Re-running one site with drop logging showed the model wraps quotes in curly quotation marks and uses straight apostrophes where sites use curly ones; the strict substring check rejected both, silently starving campaign coverage for weeks. The guard now strips the model's wrapping marks, folds typography for the comparison only, and publishes the page's own exact span; an ellipsis splice is still rejected. Denise Powell's site went 0/20 to 19/20; a backfill pass published 61 records with 3 genuine splice rejections.

**Notable Changes:**
- Live demo path tested on production: TX-22 committees question (8 citations), the vote-for-whom refusal, "who is running against Al Green", the Chronicle's address through /find (TX-7).
- Not-found pages on dynamic routes return HTTP 200 with a noindex meta because the root loading skeleton streams before the lookup; documented, not changed.
- Known-good gates at session end: 110/110 tests, lint, typecheck, build; depth eval 23/23 on both providers; sweep 56/56; health WARN, 0 crit, 0 unrecovered failures. Main at 94dc2b1 or later, pushed and deployed.
- Queued for a person: Al Green's "Juris Doctorate in 1973" quotes (his site also says 1974) as the first named rejection via `review-member-biographies.ts --reject-claim=... --reviewer=... --apply`.
...
**Session Summary:**
- Four-team parallel audit (DB/filings, cron/CI, SEO/AI-SEO, security) plus email and Vercel review, then everything actionable decided and shipped across nine commits (6697b8c..e1b0db1).
- Root-caused the $78/month Neon bill: not a poller — root-layout `force-dynamic` (for the CSP nonce) made every page render hit Postgres, and crawlers walking the 13k-URL sitemap generated ~5,400 SSR renders/day, so the DB never idled. Split the CSP in proxy.ts: nonce + strict-dynamic only on input routes (/ask, /find, /compare, /admin, /health, each force-dynamic), static policy everywhere else, ISR (~1h effective) on all content routes via layout revalidate + empty generateStaticParams. Verified x-vercel-cache HIT in prod. The footer HealthDot's 60s unstable_cache silently capped the whole site's revalidate at 1m (Next takes the route minimum) — raised to 3600.
- CI had been red since Aug 10 on two no-explicit-any errors in a one-off audit script; deleted per the no-throwaway-scripts rule. Canonical base moved to delegationdecoded.org (env var was never set; code fallbacks in layout/robots/sitemap/feed now carry the real domain) and per-page canonicals added to all indexable routes.
- Feature decisions codified in a new GOALS.md (canon, linked from AGENTS.md): portfolio proof first, users second. Press-release collection retired — it fed only a coverage-card chip after the July repositioning pointed statements at Capitol Releases, and its 16-19 min was exactly what pushed the weekly job past its 90-min ceiling (4 of 5 Sundays cancelled). House PTR vision parsing paused deliberately — one oversized McCaul filing had aborted the queue 30/30 nights; Senate eFD continues, parsed House trades stay published, /health carries dated paused-source notes.
- Data integrity: members.ts now closes terms.is_current for anyone out of office (self-healing; cleared stale rows for Graham, Scott, Cherfilus-McCormick that made them read as sitting members to /ask). schema.sql gained the five missing tables (events, votes, vote_positions, press_releases, delegation_briefs), introspected from live and matching lib/schema.ts exactly — it can now build a fresh database.
- Security batch: photo proxy checks member existence (instant 404), 300/hr per-IP limit, 6s upstream timeouts, 5MB cap, congress.gov hostname gate on the API-supplied image URL. Five CSV exports ported to votes.csv's keyset streaming, verified byte-identical including a forced 1,000-page run; filings.csv correlated COUNT became a grouped LEFT JOIN. undici/postcss overrides bumped; pnpm audit --prod now clean.
- SEO/AI visibility: JSON-LD (Person / GovernmentOrganization / Dataset / Legislation) from already-fetched data via an escaping component; /llms.txt with live DB counts and /llms-full.txt methodology; robots.txt names seven AI crawlers with explicit allows, blocks Bytespider, opens /api/photo/ and /api/data/; sitemap adds state-authority candidate pages and drops /health (now noindexed); starter SVGs removed.
- Pipeline: Indiana parser fixed (state went uppercase on headers Aug 27; normalized per-column allowlist, still fails loudly on a wrong sheet; pinned URL moved off the 8-row mid-recount July file; regression test added, suite now 83). Daily 15-min stalest-first finance pass added — weekly-only covered ~84/535 members and the 67% backlog could only grow. Ticker blacklist gained 91282CGJ4 and PRNTH plus a shape gate.

**Notable Changes:**
- Adding a new page that accepts user input now requires joining NONCE_ROUTE_PREFIXES in proxy.ts AND exporting force-dynamic; never set a shared component's cache revalidate below 3600 or it caps the whole site.
- Vercel Web Analytics was already enabled (76 visitors/wk, 91% US); the MCP 404 that suggested otherwise was wrong. PostHog recommended for location/duration/journeys when wanted.
- Deferred, on record: bill-page fattening (11.8k thin pages, largest remaining SEO item), House PTR rebuild with filing chunking, a stated license on /for-journalists for Dataset search, two legacy junk-ticker rows left in place (pages unlinked + noindexed), OG-image routes still render per request.
- Known-good gates at session end: build, lint, typecheck clean; 83/83 tests; prod verified serving split CSP, cache HITs, canonicals, JSON-LD, llms.txt.
